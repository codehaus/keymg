 ...
 * 
 * <p>
 * The <tt>options</tt> parameter, which appears in a few places, is used to pass several pieces of information to the encoder.
 * In the "higher level" methods such as encodeBytes( bytes, options ) the options parameter can be used to indicate such things
 * as first gzipping the bytes before encoding them, not inserting linefeeds, and encoding using the URL-safe and Ordered
 * dialects.
 * </p>
 * 
 * <p>
 * Note, according to <a href="http://www.faqs.org/rfcs/rfc3548.html">RFC3548</a>, Section 2.1, implementations should not add
 * line feeds unless explicitly told to do so. I've got Base64 set to this behavior now, although earlier versions broke lines
 * by default.
 * </p>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li>v2.3.7 - Fixed subtle bug when base 64 input stream contained the value 01111111, which is an invalid base 64 character
 * but should not throw an ArrayIndexOutOfBoundsException either. Led to discovery of mishandling (or potential for better
 * handling) of other bad input characters. You should now get an IOException if you try decoding something that has bad
 * characters in it.</li>
 ...
 * <li>v2.3.5 - Fixed bug in {@link #encodeFromFile} where estimated buffer size was wrong for files of size 31, 34, and 37
 * bytes.</li>
 ...
 * (by padding with equals signs) too soon. Also added an option to suppress the automatic decoding of gzipped streams. Also
 * added experimental support for specifying a class loader when using the
 ...
 * the {@link #encodeBytesToBytes(byte[])} family of methods (and not using the gzip options which uses a different mechanism
 * with streams and stuff).</li>
 * <li>v2.3.1 - Added {@link #encodeBytesToBytes(byte[], int, int, int)} and some similar helper methods to be more efficient
 * with memory by not returning a String but just a byte array.</li>
 * <li>v2.3 - <strong>This is not a drop-in replacement!</strong> This is two years of comments and bug fixes queued up and
 * finally executed. Thanks to everyone who sent me stuff, and I'm sorry I wasn't able to distribute your fixes to everyone
 * else. Much bad coding was cleaned up including throwing exceptions where necessary instead of returning null values or
 ...
 * <li><em>Removed all references to System.out, System.err, and the like.</em> Shame on me. All I can say is sorry they were
 * ever there.</li>
...
public class Base64 {
     ...
     * specify ORDERED and URL_SAFE in which case one of them will be picked, though there is no guarantee as to which one will
     * be picked.
     */
    ...
    /**
     * Performs Base64 encoding on the <code>raw</code> ByteBuffer, writing it to the <code>encoded</code> ByteBuffer. This is
     * an experimental feature. Currently it does not pass along any options (such as {@link #DO_BREAK_LINES} or {@link #GZIP}.
     * 
    ...
    /**
     * Performs Base64 encoding on the <code>raw</code> ByteBuffer, writing it to the <code>encoded</code> CharBuffer. This is
     * an experimental feature. Currently it does not pass along any options (such as {@link #DO_BREAK_LINES} or {@link #GZIP}.
     * 
     * @param raw input buffer
     * @param encoded output buffer
     ...
     * <b>This is new to v2.3!</b> In earlier versions, it just returned a null value, but in retrospect that's a pretty poor
     * way to handle it.
     * </p>
     * 
    ...
    /**
     * Similar to {@link #encodeBytes(byte[], int, int, int)} but returns a byte array instead of instantiating a String. This
     * is more efficient if you're working with I/O streams and have large data sets to encode.
     * 
     * 
     * @param source The data to convert
     * @param off Offset in array where conversion should begin
